...
P4VOID p4_lock_init(p4_lock_t *l) {
    *l = 0;
}

P4VOID p4_lock(p4_lock_t *l) {
    trap(ENTER_ACQUIRE, l);
...
P4VOID p4_unlock(p4_lock_t *l) {
    trap(ENTER_RELEASE, l);
...
int p4_moninit(p4_monitor_t *m, int i) {
    int j;
...
P4VOID p4_menter(p4_monitor_t * m)
{
...
P4VOID p4_mexit(p4_monitor_t *m)
{
...
P4VOID p4_mdelay(p4_monitor_t *m, int i)
{
    struct p4_mon_queue *q;
...
P4VOID p4_mcontinue(p4_monitor_t *m, int i)
{
    struct p4_mon_queue *q;
...
int num_in_mon_queue(p4_monitor_t *m, int i)
{
    struct p4_mon_queue *q;
...
int p4_getsub_init(p4_getsub_monitor_t *gs)
{
...
P4VOID p4_getsubs(p4_getsub_monitor_t *gs, int *s, int max, 
...
int p4_barrier_init(p4_barrier_monitor_t *b)
{
    trap(BARRIER_INIT, b);
    b->count = 0;
    return (p4_moninit(&(b->m), no_procs));
}

P4VOID p4_barrier(p4_barrier_monitor_t *b, int nprocs) {

    trap(ENTER_BARRIER, b, nprocs);
...
int p4_askfor(p4_askfor_monitor_t *af, int nprocs, 
	      int (*getprob_fxn)(), P4VOID *problem, 
	      P4VOID (*reset_fxn)())
{
    int rc;
...
P4VOID p4_update(p4_askfor_monitor_t *af, int (*putprob_fxn)(), 
		 P4VOID *problem)
{
    p4_menter(&(af->m));
...
P4VOID p4_probend(p4_askfor_monitor_t *af, int code)
{
    p4_menter(&(af->m));
    af->pbdone = code;
...
P4VOID p4_progend(p4_askfor_monitor_t *af)
{
    p4_menter(&(af->m));
    af->pgdone = 1;
